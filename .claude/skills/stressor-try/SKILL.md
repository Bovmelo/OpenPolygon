...
argument-hint: [path_to_problem_statement_or_problem_dir]
...
The local Claude Code agent must not solve the problem directly.
...
4. Invokes `.claude/skills/stressor-try/scripts/api_try.py`.
...
```text
.claude/skills/stressor-try/scripts/api_try.py
```
...
* do not use local Claude Code reasoning to fill code
...
```text
.claude/skills/stressor-try/scripts/api_init.sh
```
...
```bash
source .claude/skills/stressor-try/scripts/api_init.sh
python3 .claude/skills/stressor-try/scripts/api_try.py ...
```
...
```text
Missing .claude/skills/stressor-try/scripts/api_init.sh
```
...
The local Claude Code agent must not produce the solution code itself.
...
* Must invoke `.claude/skills/stressor-try/scripts/api_try.py`.
* Must source `.claude/skills/stressor-try/scripts/api_init.sh` before invoking `api_try.py`.
